...
def update_json_file(file_path, mapping):
    ...
    if github_compare_link and 'https://github.com' in github_compare_link:
        repo_name = get_repo_name_from_url(github_compare_link)
    else:
        ...
        if repo_info:
            repo_name = get_repo_name_from_url(repo_info["githubRepoLink"])

    if repo_name:
        license_info = get_license_info(repo_name)
        updatedDependency['licenseInfo'] = license_info
        updatedDependency['githubRepoSlug'] = repo_name
